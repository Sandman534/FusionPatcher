...
namespace Fusion
{
    internal class BOOK
    {
        public static void Patch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state, SettingsUtility Settings)
        {
            ...
            HashSet<FormKey> affectedFormKeys = Utility.GetAffectedFormKeys<IBookGetter>(state, workingModList);
            Utility.RecordCountMessage(affectedFormKeys.Count, "Book");
            ...
            foreach (var formKey in affectedFormKeys)
            {
                ...
                var allContexts = state.LinkCache.ResolveAllContexts<IBook, IBookGetter>(formKey).ToList();
                ...
                foreach(var fContext in modContext)
                {
                    ...
                    if (Utility.TagCheck(Tags.Graphics, mapped, Settings, fContext))
                    {
                        if (
                            ...
                            || Compare.NotEqual(fContext.Record.InventoryArt,oContext.Record.InventoryArt)
                            ...
                            if (Utility.CheckContext(fContext, wContext, oContext)) {
                                ...
                                if (Utility.ShouldChange(fContext.Record.InventoryArt,wContext.Record.InventoryArt,oContext.Record.InventoryArt)) {
                                    overrideObject ??= wContext.GetOrAddAsOverride(state.PatchMod);
                                    overrideObject.InventoryArt.SetTo(fContext.Record.InventoryArt);
                                }
                    ...
                    if (Utility.TagCheck(Tags.Text, mapped, Settings, fContext))
                    {
                        if (
                            ...
                            || Compare.NotEqual(fContext.Record.BookText,oContext.Record.BookText)
                        ){
                            if (Utility.CheckContext(fContext, wContext, oContext)) {
                            ...
                            if (Utility.CheckContext(fContext, wContext, oContext)) {
                                if (Utility.ShouldChange(fContext.Record.BookText,wContext.Record.BookText,oContext.Record.BookText)) {
                                    overrideObject ??= wContext.GetOrAddAsOverride(state.PatchMod);
                                    overrideObject.BookText = Utility.NewStringNotNull(fContext.Record.BookText);
                                }
                            }
                            mapped.SetMapped();
                        }
