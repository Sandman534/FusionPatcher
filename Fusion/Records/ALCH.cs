...
namespace Fusion
{
    internal class ALCH
    {
        public static void Patch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state, SettingsUtility Settings)
        {
            ...
            HashSet<FormKey> affectedFormKeys = Utility.GetAffectedFormKeys<IIngestibleGetter>(state, workingModList);
            Utility.RecordCountMessage(affectedFormKeys.Count, "Potion");
            ...
            foreach (var formKey in affectedFormKeys)
            {
                ...
                var allContexts = state.LinkCache.ResolveAllContexts<IIngestible, IIngestibleGetter>(formKey).ToList();
                ...
                // Tracking Tags
                IIngestible? overrideObject = null;
                ...
                foreach(var fContext in modContext)
                {
                    ...
                    if (Utility.TagCheck(Tags.Sound, mapped, Settings, fContext))
                    {
                        if (
                            ...
                            || Compare.NotEqual(fContext.Record.ConsumeSound,oContext.Record.ConsumeSound)
                        ){
                            if (Utility.CheckContext(fContext, wContext, oContext)) {
                                ...
                                if (Utility.ShouldChange(fContext.Record.ConsumeSound,wContext.Record.ConsumeSound,oContext.Record.ConsumeSound)) {
                                    overrideObject ??= wContext.GetOrAddAsOverride(state.PatchMod);
                                    overrideObject.ConsumeSound.SetTo(fContext.Record.ConsumeSound);
                                }
                            }
                            mapped.SetMapped();
                        }
