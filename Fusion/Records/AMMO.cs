...
namespace Fusion
{
    internal class AMMO
    {
        public static void Patch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state, SettingsUtility Settings)
        {
            ...
            HashSet<ModKey> workingModList = Settings.GetModList(Tags.Destructible, Tags.Graphics, Tags.Keywords, Tags.Names, Tags.ObjectBounds, Tags.Sound, Tags.Stats, Tags.Text);
            HashSet<FormKey> affectedFormKeys = Utility.GetAffectedFormKeys<IAmmunitionGetter>(state, workingModList);
            Utility.RecordCountMessage(affectedFormKeys.Count, "Ammo");
            ...
            foreach (var formKey in affectedFormKeys)
            {
                ...
                var allContexts = state.LinkCache.ResolveAllContexts<IAmmunition, IAmmunitionGetter>(formKey).ToList();
                ...
                // Tracking Tags
                IAmmunition? overrideObject = null;
                ...
                foreach(var fContext in modContext)
                {
                    ...
                    {
                        if (Compare.NotEqual(fContext.Record.PickUpSound,oContext.Record.PickUpSound)
                    ...
                    if (Utility.TagCheck(Tags.Stats, mapped, Settings, fContext))
                    {
                        if (
                            ...
                            || Compare.NotEqual(fContext.Record.Damage,oContext.Record.Damage)
                            ...
                            if (Utility.CheckContext(fContext, wContext, oContext)) {
                                ...
                                if (Utility.ShouldChange(fContext.Record.Damage,wContext.Record.Damage,oContext.Record.Damage)) {
                                    overrideObject ??= wContext.GetOrAddAsOverride(state.PatchMod);
                                    overrideObject.Damage = fContext.Record.Damage;
                                }
                    ...
                    if (Utility.TagCheck(Tags.Text, mapped, Settings, fContext))
                    {
                        if (
                            ...
                            || Compare.NotEqual(fContext.Record.ShortName,oContext.Record.ShortName)
                        ){
                            if (Utility.CheckContext(fContext, wContext, oContext)) {
                                ...
                                if (Utility.ShouldChange(fContext.Record.ShortName,wContext.Record.ShortName,oContext.Record.ShortName)) {
                                    overrideObject ??= wContext.GetOrAddAsOverride(state.PatchMod);
                                    overrideObject.ShortName = fContext.Record.ShortName;
                                }
                            }
                            mapped.SetMapped();
                        }                        
