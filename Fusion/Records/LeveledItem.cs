            ...
            foreach (var workingContext in state.LoadOrder.PriorityOrder.LeveledItem().WinningContextOverrides())
            {
                ...
                if (Settings.TagCount("Relev", out var FoundKeys) > 0)
                {
                    ...
                    if (foundContext.Any())
                    {
                        ...
                        ExtendedList<LeveledItemEntry> overrideObject = new();
                        if (state.LinkCache.TryResolveContext<ILeveledItem, ILeveledItemGetter>(workingContext.Record.FormKey, out var patchRecord) && patchRecord.Record.Entries != null)
