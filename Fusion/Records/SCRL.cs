...
namespace Fusion
{
    internal class SCRL
    {
        public static void Patch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state, SettingsUtility Settings)
        {
            ...
            HashSet<ModKey> workingModList = Settings.GetModList(Tags.Destructible, Tags.Graphics, Tags.Keywords, Tags.Names, Tags.ObjectBounds, Tags.Sound, Tags.SpellStats, Tags.Text);
            HashSet<FormKey> affectedFormKeys = Utility.GetAffectedFormKeys<IScrollGetter>(state, workingModList);
            Utility.RecordCountMessage(affectedFormKeys.Count, "Scroll");
            ...
            foreach (var formKey in affectedFormKeys)
            {
                ...
                var allContexts = state.LinkCache.ResolveAllContexts<IScroll, IScrollGetter>(formKey).ToList();
